...
def get_initial(info, fixed: IO):
    """Get initial state from a CSV file."""
    initial = {}
...
def write_sif(sbml_filename: str, info, graph: nx.DiGraph):
    ...
    with open(sbml_filename[:-4] + "sif", "w", encoding="utf-8", newline="") as f:
        print(f"# Generated with CaSQ v{version}", file=f)
        with open(
            sbml_filename[:-5] + "_raw.sif", "w", encoding="utf-8", newline=""
        ) as fraw:
            print(f"# Generated with CaSQ v{version}", file=fraw)
...
def write_csv(sbml_filename: str, info, fixed: Optional[IO] = None):
    """Write CSV files with Spreadsheet-SBML-qual information."""
